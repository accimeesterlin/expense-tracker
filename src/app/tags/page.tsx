...
const predefinedColors = [
  "#006BFF", "#10B981", "#F59E0B", "#EF4444", "#8B5CF6", 
      ...
      if (response.ok) {
        ...
        if (editingTag) {
          setTags(prev => prev.map(tag => 
            tag._id === editingTag._id ? result : tag
          ));
        } else {
                ...
                <div>
                  ...
                  <div className="flex items-center space-x-2">
                    <input
                      ...
                      className="w-10 h-10 rounded border border-[#E5E7EB] cursor-pointer"
                    />
                      ...
                      {predefinedColors.map(color => (
                        <button
                          ...
                          className={`w-6 h-6 rounded border-2 ${
                            formData.color === color ? 'border-[#0B3558]' : 'border-transparent'
                          }`}
        ...
        <div className="card">
          ...
          {tags.length === 0 ? (
            <div className="p-12 text-center">
              <Hash className="w-12 h-12 text-[#A6BBD1] mx-auto mb-4" />
              <p className="text-[#476788]">No tags created yet</p>
              <p className="text-sm text-[#A6BBD1]">Use the form above to create your first tag</p>
            </div>
          ) : (
            <div className="p-6">
              <div className="flex flex-wrap gap-3">
                {tags.map((tag) => (
                  <div
                    key={tag._id}
                    className="inline-flex items-center space-x-2 px-3 py-2 rounded-full border border-[#E5E7EB] bg-white group hover:shadow-sm transition-shadow"
                  >
                    <div
                      className="w-3 h-3 rounded-full"
                      style={{ backgroundColor: tag.color }}
                    />
                    <span className="text-sm font-medium text-[#0B3558]">{tag.name}</span>
                    <div className="flex items-center space-x-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button
                        onClick={() => handleEdit(tag)}
                        className="p-1 text-[#476788] hover:text-[#0B3558] rounded transition-colors"
                      >
